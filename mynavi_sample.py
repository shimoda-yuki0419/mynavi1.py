...
def main():
    ...
    contens_list=driver.find_elements_by_class_name("tableCondition__body")
    # 1ページ分繰り返し
    print("{},{},{},{}".format(len(copy_list),len(status_list),len(name_list),len(contens_list)))
    for name,copy,status,contens in zip(name_list,copy_list,status_list,contens_list):
        print(name.text)
        print(copy.text)
        print(status.text)
        print(contens.text)
